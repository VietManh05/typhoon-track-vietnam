...
def make_bundle(tmp_path):
    ...
    config = {
        "n_features": len(FEATURE_COLUMNS), "n_horizons": 5, "n_classes": 7,
        "hidden_size": 8, "num_layers": 1, "dropout": 0.0,
    }
    model = LSTMTrackForecaster(**config)
    export_bundle(
        tmp_path / "bundle", model, scaler, version="test-v1",
        dataset_version="synthetic-v1", dataset_sha256="a" * 64,
        model_config=config, input_len=4, horizons_hours=[6, 12, 24, 48, 72],
...
def frame_to_fixes(frame: pd.DataFrame) -> list[Fix]:
    return [
        Fix(timestamp=row.timestamp, lat=row.lat, lon=row.lon, wind_ms=row.wind_ms,
            pressure_hpa=row.pressure_hpa, intensity=row.intensity, source="synthetic-demo")
...
def test_bundle_round_trip_and_horizon_mapping(tmp_path) -> None:
    ...
    coords, classes = predict_bundle(bundle, frame_to_fixes(frame.iloc[:8]), [6, 24, 72])
...
def test_predict_rejects_irregular_window(tmp_path) -> None:
    ...
    fixes[-2] = fixes[-2].model_copy(update={"timestamp": fixes[-2].timestamp + timedelta(hours=1)})
