...
metadata = MetaData()
storms = Table("typhoons", metadata, Column("id", String(80), primary_key=True),
...
forecasts = Table("forecasts", metadata,
    Column("id", String(64), primary_key=True), Column("storm_id", String(80), index=True),
    Column("issue_time", String(40), index=True), Column("generated_at", String(40)),
    Column("payload", Text, nullable=False))
subscriptions = Table("subscriptions", metadata,
    Column("id", String(40), primary_key=True), Column("payload", Text, nullable=False))
alerts = Table("alerts", metadata,
    Column("id", String(64), primary_key=True), Column("subscription_id", String(40)),
    Column("storm_id", String(80)), Column("created_at", String(40)), Column("payload", Text))
...
class Store:
        ...
        with self.lock, self.engine.begin() as conn:
            ...
            for fix in batch.observations:
                statement = upsert(observations).values(
                    storm_id=batch.storm_id, timestamp=fix.timestamp.isoformat(),
    ...
    def track(self, storm_id):
        ...
        with self.engine.connect() as conn:
            rows = conn.execute(select(observations.c.payload)
                .where(observations.c.storm_id == storm_id)
                .order_by(observations.c.timestamp)).scalars().all()
    ...
    def active(self, hours=48):
        ...
        for entry in entries:
            ...
            if track and track[-1].timestamp >= threshold:
                results.append({**dict(entry), "latest": track[-1].model_dump(mode="json"),
                                "observation_count": len(track)})
        return results

    def cached(self, key):
        with self.engine.connect() as conn:
            payload = conn.execute(select(forecasts.c.payload)
                                   .where(forecasts.c.id == key)).scalar()
    ...
    def save_forecast(self, forecast):
        ...
        with self.engine.begin() as conn:
            statement = upsert(forecasts).values(
                id=forecast.forecast_id, storm_id=forecast.storm_id,
    ...
    def latest_forecast(self, storm_id, issue_time=None):
        ...
        with self.engine.connect() as conn:
            payload = conn.execute(query.order_by(
                forecasts.c.issue_time.desc(), forecasts.c.generated_at.desc())).scalar()
        return json.loads(payload) if payload else None
    ...
    def alert_drafts(self):
        with self.engine.connect() as conn:
            return [json.loads(r) for r in conn.execute(
                select(alerts.c.payload).order_by(alerts.c.created_at.desc()).limit(100)).scalars()]

    def record_alert(self, subscription_id, forecast, subscription, closest):
        import hashlib
        now = datetime.now(timezone.utc)
        key = hashlib.sha256(
            (subscription_id + forecast["forecast_id"]).encode()).hexdigest()
        with self.lock, self.engine.begin() as conn:
            # Lock subscription row across PostgreSQL workers.
            conn.execute(select(subscriptions.c.id).where(
                subscriptions.c.id == subscription_id).with_for_update()).first()
            previous = conn.execute(select(alerts.c.created_at).where(
                alerts.c.subscription_id == subscription_id,
                alerts.c.storm_id == forecast["storm_id"]).order_by(
                alerts.c.created_at.desc())).scalar()
            if previous and now-datetime.fromisoformat(previous) < timedelta(
                    hours=subscription["cooldown_hours"]):
                return False
            draft = {"id": key, "label": subscription["label"],
                     "storm_id": forecast["storm_id"], "forecast_id": forecast["forecast_id"],
                     "created_at": now.isoformat(), "closest_km": closest,
                     "status": "draft", "disclaimer": forecast["disclaimer"],
                     "message": "Forecast enters your configured radius. Consult NCHMF.",
                     "model_version": forecast["model_version"], "sources": forecast["sources"]}
            from sqlalchemy.dialects.postgresql import insert as pg_insert
            from sqlalchemy.dialects.sqlite import insert as sqlite_insert
            upsert = sqlite_insert if self.engine.dialect.name == "sqlite" else pg_insert
            result = conn.execute(upsert(alerts).values(
                id=key, subscription_id=subscription_id, storm_id=forecast["storm_id"],
                created_at=now.isoformat(), payload=json.dumps(draft))
