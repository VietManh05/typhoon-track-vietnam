...
from typhoon_vn.datasets.typhoon_dataset import DatasetConfig, TyphoonDataset, collate_fn, split_by_storm_or_year
...
def _frame_hash(frame: pd.DataFrame) -> str:
    stable = frame.sort_values(["storm_id", "timestamp"]).to_json(date_format="iso", orient="records")
...
def run_training(
    ...
    *,
    ...
    if frame is None:
        frame = generate_synthetic_catalogue(n_storms=8, n_fixes=20, seed=config.experiment.seed)
    train, val, test = split_by_storm_or_year(
        frame, val_ratio=config.dataset.val_ratio, test_ratio=config.dataset.test_ratio,
        by_year=config.dataset.split_by_year, random_seed=config.dataset.random_seed,
    )
    if train.empty or val.empty:
        raise ValueError("training and validation splits must be non-empty")
    split_manifest = {
        "seed": config.dataset.random_seed, "train": _ids(train), "val": _ids(val),
        "test": _ids(test), "dataset_sha256": _frame_hash(frame),
    }
    ...
    dataset_config = DatasetConfig(
        input_len=config.dataset.input_len, horizon=tuple(config.dataset.horizon),
    ...
    model_config = {
        "n_features": len(FEATURE_COLUMNS), "n_horizons": len(config.dataset.horizon),
        "n_classes": train_ds.n_classes, "hidden_size": config.model.hidden_size,
    ...
    loss = MultiHorizonLoss(
        len(config.dataset.horizon), train_ds.n_classes, reg_loss=config.loss.reg_loss,
        horizon_weights=config.loss.horizon_weights, reg_weight=config.loss.reg_weight,
    ...
    trainer = Trainer(
        ...
        TrainingConfig(
            ...
            patience=config.training.patience, gradient_clip=config.training.gradient_clip,
            scheduler=config.training.scheduler, device=config.experiment.device,
            checkpoint_dir=output_dir / "checkpoints", log_every=config.training.log_every,
        ),
        loss,
    )
    result = trainer.fit(train_loader, val_loader)
    # Smoke artifact: radii are explicitly illustrative, not a real calibration claim.
    horizons_hours = [step * dataset_config.time_step_hours for step in config.dataset.horizon]
    radii = {str(hour): max(30.0, hour * 5.0) for hour in horizons_hours}
    manifest = export_bundle(
        output_dir / "bundle", model, scaler, version=config.experiment.name,
        dataset_version=dataset_version, dataset_sha256=split_manifest["dataset_sha256"],
        model_config=model_config, input_len=config.dataset.input_len,
        horizons_hours=horizons_hours, validation_radius_km=radii,
        split_manifest=split_manifest, synthetic=dataset_version.startswith("synthetic"),
    )
    load_bundle(manifest)
    (output_dir / "result.json").write_text(
        json.dumps({"history": result["history"], "manifest": str(manifest)}, indent=2),
        encoding="utf-8",
    )
    return {"manifest": manifest, "history": result["history"], "split": split_manifest}


def run_training_file(config_path: str | Path, output_dir: str | Path) -> dict[str, Any]:
