...
from typhoon_vn.api.schemas import (DISCLAIMER, ForecastRequest, ForecastResponse,
                                    ObservationBatch, Subscription)
...
from typhoon_vn.settings import get_settings

logger = logging.getLogger(__name__)

def create_app(settings=None, store=None, forecaster=None) -> FastAPI:
    ...
    if settings.app_env == "production" and (not settings.api_key or settings.allow_baseline):
        raise ValueError("Production requires API_KEY and ALLOW_BASELINE=false")
    app = FastAPI(title="Typhoon VN Forecast System", version=__version__,
    ...
    def ready():
        ...
        try:
            ...
            return {"status": "ready", "model_version": initialize()["forecaster"].version}
        except Exception:
    ...
    def version():
        ...
        return {"version": __version__, "model_version": service.version,
                "model_kind": service.kind, "dataset_version": service.dataset}

    @app.get("/metrics", response_class=PlainTextResponse, dependencies=[Depends(authorize)])
    def metrics():
        return "\n".join(f"typhoon_{name}_total {value}" for name, value in counters.items())+"\n"
    ...
    def compute(body):
        ...
        if services["redis"]:
            try:
                services["redis"].setex("forecast:"+key, 1800, result.model_dump_json())
            except Exception:
                logger.warning("Redis unavailable; durable database cache remains active")
        return result

    @app.post("/forecast", response_model=ForecastResponse, dependencies=[Depends(authorize)])
    ...
    def track(storm_id: str):
        ...
        if not fixes:
            raise HTTPException(404, "Storm not found")
        return {"storm_id": storm_id, "observations": fixes,
                "forecast": compute(ForecastRequest(storm_id=storm_id)) if len(fixes)>1 else None}
    ...
    def impact(storm_id: str, lat: float, lon: float):
        ...
        points = [{"horizon_hours": p.horizon_hours,
                   "distance_km": haversine_km(lat, lon, p.lat, p.lon),
                   "within_illustrative_radius": haversine_km(lat, lon, p.lat, p.lon) <= p.radius_km}
                  for p in result.points]
        return {"storm_id": storm_id, "location": {"lat": lat, "lon": lon},
                "points": points, "disclaimer": DISCLAIMER,
